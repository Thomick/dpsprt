...
- `DPSPRTTuned` rejected `c2 < 1`, which ruled out the regime the paper's own tuned experiment uses. Privacy is unaffected by `c2`, since the noise does not change; only the `(alpha, beta)` guarantee needs `c2 >= 1`. Values below 1 are now allowed and warn, and a test reproduces the paper's `kappa = 0.5` result.
